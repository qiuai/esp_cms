...
class connector {
	...
	function start_pagetemplate($lng_templates = null, $lan_pack = array()) {
		...
		$this->pagetemplate->libtype = 'web';
		$this->pagetemplate->caching = false;
		$this->pagetemplate->cache_time = 86400;
		$this->pagetemplate->isdbo = $this->CON['isdbo'];
	...
	function get_lng_array($lng = null, $isuptype = 0, $iswap = 0) {
		...
		if ($iswap == 1) {
			$db_where .= " AND iswap=1";
		}
		$lngarray = $this->dbcache->checkcache('lng_array_' . $isuptype . '_' . $iswap, false);
		if (!$lngarray) {
			...
			$lngarray = $this->dbcache->cachesave('lng_array_' . $isuptype . '_' . $iswap, $array);
	...
	function get_tempmail_view($tecode = null, $styleclass = 0, $returnname = null, $lng = null) {
		...
		$chacheview = $this->dbcache->checkcache('templates_view_' . $tecode . '_' . $lng, false);
		if (!$chacheview) {
			$rsPuv = $this->db->fetch_first('SELECT * FROM ' . $db_table . ' WHERE ' . $db_where);
			$chacheview = $this->dbcache->cachesave('templates_view_' . $tecode . '_' . $lng, $rsPuv);
			$chacheview = $chacheview ? $chacheview : $rsPuv;
			unset($rsPuv);
		}
		if (!empty($returnname)) {
	...
	function articlehtml($did, $smilan = '') {
		...
		if (empty($readinfo['filename']) || empty($readinfo['filepath'])) {
			...
			$filename = $readinfo['isbase'] ? $entrance_file : $readinfo['filename'];
			$readinfo['filename'] = $filename;
	...
	function get_moblie_type_array($mobtid = 0) {
		...
		$sql = 'SELECT * FROM ' . $db_table . $db_where;
		$chacherray = $this->dbcache->checkcache('moblie_type_array', false);
		$arrayList = array();
		if (!$chacherray) {
			$rs = $this->db->query($sql);
			while ($rsList = $this->db->fetch_assoc($rs)) {
			...
			$chacherray = $this->dbcache->cachesave('moblie_type_array', $array);
			$chacherray = $chacherray ? $chacherray : $array;
			unset($array);
		}
		$chacherray = $this->fun->reset_array($chacherray, $mobtid, 'mobtid');
		$i = count($chacherray);
		$arrayList['num'] = $i;
		$arrayList['list'] = $chacherray;
		return $arrayList;
	...
	function membersmssend($array = array(), $tomoblie = 0, $smscode = null) {
		...
		$templatecontent = str_replace('[%ordersn%]', $array['ordersn'], $templatecontent);
		$templatecontent = str_replace('[%enquirysn%]', $array['enquirysn'], $templatecontent);
	...
	function memberlink($read = array(), $lng_temp = '', $is_wap = 0) {
		...
		if ($is_rewrite) {
			...
			$link['moblie_validatesave'] = 'membermain_moblievalidatesave' . $file_fileex;
			$link['moblie_getvalidatecode'] = 'membermain_getvalidatecode' . $file_fileex;
		} else {
			...
			$link['moblie_validate'] = "index.php?ac=membermain&at=moblievalidate";
