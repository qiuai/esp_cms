...
class important extends connector {
	...
	function onseosave() {
		...
		$linknum = empty($linknum) ? 500 : $linknum;
	...
	function onbaidusave() {
		...
		while ($rsList = $this->db->fetch_assoc($rs)) {
			...
			$xmlfile.="\x20\x20\x20\x20\x20\x20\x20\x20<priority>1.0</priority>\n";
			$xmlfile.="\x20\x20\x20\x20</url>\n";
		}
		$xmlfile .= '</urlset>';
		$commandfile = admin_ROOT . $file_sitemapdir . '/baidusitemap.xml';
		if ($this->fun->filemode(admin_ROOT . $file_sitemapdir)) {
			if (!is_dir(admin_ROOT . $file_sitemapdir)) {
				if (!@mkdir(admin_ROOT . $file_sitemapdir, 0777, true)) {
					exit($file_sitemapdir . $this->lng['createmain_creat_no_w']);
		...
		if (!$this->fun->filewrite($commandfile, $xmlfile)) {
			exit($this->lng['createmain_creat_no'] . $file_sitemapdir . '/baidusitemap.xml');
		} else {
			exit('true');
		}
	}
