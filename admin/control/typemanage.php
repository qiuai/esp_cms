...
class important extends connector {
	...
	function ontypelist() {
		...
		$limitkey = empty($limitkey) ? 'pid' : $limitkey;
		$limitclass = $this->fun->accept('limitclass', 'R');
		$limitclass = empty($limitclass) ? 'ASC' : $limitclass;
		$sql = 'SELECT * FROM ' . $db_table . $db_where . ' ORDER BY ' . $limitkey . ' ' . $limitclass . ',tid';
		...
		while ($rsList = $this->db->fetch_assoc($rs)) {
			...
			switch ($rsList['styleid']) {
				case 1:
					$rsList['stylename'] = '<span style="color:#489D66">' . $this->lng['typemanage_text_styleid1'] . '</span>';
					break;
				case 2:
					$rsList['stylename'] = $this->lng['typemanage_text_styleid2'];
					break;
				case 3:
					$rsList['stylename'] = '<span style="color:#FF0F0F">' . $this->lng['typemanage_text_styleid3'] . '</span>';
					break;
				case 4:
					$rsList['stylename'] = '<span style="color:#2E43E0">' . $this->lng['typemanage_text_styleid4'] . '</span>';
					break;
			}
			$t_array[] = $rsList;
		}
		$threearray = $this->fun->getTree($t_array);
		unset($this->CON, $this->lng);
		$this->ectemplates->assign('array', $threearray);
	...
	function ontypeshift() {
		...
		$db_where = 'tid=' . $tid;
		$typeread = $this->db->fetch_first('SELECT * FROM ' . $db_table . ' WHERE ' . $db_where);
		$typelist = $this->get_typeselect($typeread['mid'], 0, $typeread['upid'], $typeread['lng'], 0, 1);
		$this->ectemplates->assign('typelist', $typelist);
		$model = $this->get_modelview($typeread['mid'], 'modelname');
		$this->ectemplates->assign('model', $model);
		$lngdir = $this->get_lng_dirpack($typeread['lng']);
