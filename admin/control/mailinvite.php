...
class important extends connector {
	...
	function onmailinviteinput() {
		...
		$this->ectemplates->assign('read', $read);
		$this->ectemplates->assign('tab', $tab);
		$outtemplates_file = $type == 'member' ? 'mailinvite/mailinvite_mail_memberadd' : 'mailinvite/mailinvite_mail_batadd';
	...
	function onbatupinfosave() {
		...
		for ($i = 0; $data = $this->fun->fgetcsv_reg($handle); $i++) {
			if ($i > 0) {
				...
				$db_where = " WHERE mlvid=$mlvid AND email='$email'";
				$countnum = $this->db_numrows($db_table, $db_where);
				if ($countnum <= 0 && preg_match("/^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/i", $email)) {
	...
	function onmailinvitesave() {
		$mlvid = $this->fun->accept('mlvid', 'P');
		$mcid = $this->fun->accept('mcid', 'P');
		if (empty($mlvid) || empty($mcid)) exit('false');
		$time = time();
		...
		$rs = $this->db->query($sql);
		while ($rsList = $this->db->fetch_assoc($rs)) {
			$db_where = " WHERE mlvid=$mlvid AND email='$rsList[email]'";
			$countnum = $this->db_numrows($db_table2, $db_where);
			if ($countnum <= 0 && preg_match("/^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/i", $rsList['email'])) {
				$db_field = 'mlvid,userid,name,email,sex,tel,address,isclass,addtime';
				$name = empty($rsList['alias']) ? $rsList['username'] : $this->fun->daddslashes($rsList['alias'], 1);
				$tel = $this->fun->daddslashes($rsList['tel'], 1);
				$address = $this->fun->daddslashes($rsList['address'], 1);
				$db_values = "$mlvid,$rsList[userid],'$name','$rsList[email]',$rsList[sex],'$tel','$address',1,$time";
				$this->db->query('INSERT INTO ' . $db_table2 . ' (' . $db_field . ') VALUES (' . $db_values . ')');
			}
		}
		$this->writelog($this->lng['mailinvite_mail_add_log2'] . "(MCID=$mcid)", $this->lng['log_extra_ok']);
		exit('true');
	}
