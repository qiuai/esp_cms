...
class important extends connector {
	...
	function onarticlelist() {
		...
		if (!empty($serchekey)) {
			$db_where.=" AND title like '%$serchekey%' OR tsn like '%$serchekey%' ";
		}
	...
	function ondocadd() {
		...
		if ($modelview['istsn']) {
			...
			$tsn = $this->get_htmlfilename($modelview['tsnstyle'], $tsnArray);
		}
	...
	function onsetting() {
		...
		if ($dbname == 'dlid') {
			...
			$count = count($infoarray) - 1;
			if ($count <= 0) exit('false');
			$recommend = $this->fun->accept('recommend', 'R');
			$recommend = empty($recommend) ? 0 : $recommend;
			$db_table = db_prefix . 'document_label';
			$db_table2 = db_prefix . 'document';
			$db_where = 'dlid=' . $value;
			$recomread = $this->db->fetch_first('SELECT dlid,mid FROM ' . $db_table . ' WHERE ' . $db_where);
			$mid = $recomread['mid'];
			for ($i = 0; $i < $count; $i++) {
				$recommend_array = null;
				$recommendvalue = null;
				$db_where_did = $recommend ? "did=$infoarray[$i] AND mid=$mid" : "did=$infoarray[$i] AND mid=$mid AND FIND_IN_SET('$value',recommend)";
				$read = $this->db->fetch_first('SELECT did,mid,recommend FROM ' . $db_table2 . ' WHERE ' . $db_where_did);
				if ($read) {
					$recommend_array = $read['recommend'] ? explode(',', $read['recommend']) : null;
					if ($recommend) {
						if (is_array($recommend_array)) {
							if (!in_array($value, $recommend_array)) {
								$recommend_array[] = $value;
								$recommendvalue = implode(',', $recommend_array);
							}
						} else {
							$recommendvalue = $value;
						}
						if ($recommendvalue) {
							$db_set = "recommend='$recommendvalue'";
							$db_where_up = "did=$infoarray[$i]";
							$this->db->query('UPDATE ' . $db_table2 . ' SET ' . $db_set . ' WHERE ' . $db_where_up);
						}
					}
					if (!$recommend && is_array($recommend_array) && in_array($value, $recommend_array)) {
						$keyarray = array_search($value, $recommend_array);
						unset($recommend_array[$keyarray]);
						if (is_array($recommend_array)) {
							$recommendvalue = implode(',', $recommend_array);
						}
						$db_set = "recommend='$recommendvalue'";
