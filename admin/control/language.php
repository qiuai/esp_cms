...
class important extends connector {
	...
	function onlngpackcopy() {
		...
		switch ($type) {
			case 'lng':
				$templatefile = 'admin/admin_lng_packcopy';
				break;
			case 'mail':
				$templatefile = 'admin/admin_lng_mailpackcopy';
				break;
			case 'sms':
				$templatefile = 'admin/admin_lng_smspackcopy';
				break;
		}
	...
	function ontemplatepackcopy() {
		...
		$out = $this->templatescopy($lng, $tolng, $styleclass);
		if ($out) {
			if ($styleclass == 3) {
				$this->writelog($this->lng['language_mailcopy_log'], $this->lng['log_extra_ok'] . $lng . ' TO ' . $tolng);
			}
			if ($styleclass == 4) {
				$this->writelog($this->lng['language_smscopy_log'], $this->lng['log_extra_ok'] . $lng . ' TO ' . $tolng);
			}
			$this->creat_lanpack($tolng, true);
			exit('true');
		} else {
	...
	function templatescopy($lng, $tolng, $styleclass = 3) {
		...
		$db_where = " WHERE lng='$lng' AND styleclass=$styleclass ORDER BY tmid";
		...
		while ($rsList = $this->db->fetch_assoc($rs)) {
			...
			if ($countnum <= 0) {
				$db_values = "'$tolng','$rsList[templatename]','$rsList[templatecode]','$rsList[title]','$rsList[templatecontent]','$rsList[pic]','$rsList[typeclass]',$styleclass,$rsList[lockin],$date";
				$this->db->query('INSERT INTO ' . $db_table . ' (' . $db_field . ') VALUES (' . $db_values . ')');
			}
		}
		return true;
