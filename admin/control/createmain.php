...
class important extends connector {
	...
	function onstatmidtype() {
		$lng = $this->fun->accept('lng', 'R');
		$lng = ($lng == 'big5') ? $this->CON['is_lancode'] : $lng;
		$db_where = " WHERE isclass=1 AND pageclass=1 AND styleid<>3 AND lng='$lng'";
		$midlist = $this->fun->accept('midlist', 'P');
		if (!empty($midlist)) {
			$db_where.=" AND mid IN($midlist)";
		}
		...
		$sql = 'SELECT tid FROM ' . $db_table . $db_where;
		$rs = $this->db->query($sql);
		while ($rsList = $this->db->fetch_assoc($rs)) {
			$tidlist.=$rsList['tid'] . ',';
		}
		if (!empty($tidlist)) {
			$tidlist = substr($tidlist, 0, strlen($tidlist) - 1);
		}
		exit($tidlist);
	}
	function onstatype() {
		...
		$isbattype = $this->fun->accept('isbattype', 'R');
		...
		if (($creattype == 1) || $isbattype) {
			$typelist = $this->get_typeid($tid, '', 0, $mid, 0, $lng, 0, 1, 'text', 0, 1);
			if (!empty($typelist)) {
	...
	function oncreattypesave() {
		...
		$tid = ceil($this->fun->accept('tid', 'R'));
		if (empty($tid)) exit('false');
		$typeread = $this->get_type($tid);
		if (!$typeread['pageclass']) exit('false');
		...
		if ($styleid != 4) {
			if (!empty($typeread['headtitle'])) {
				$LANPACK['sitename'] = $typeread['headtitle'];
			} else {
				if ($modelview['listtitlestyle']) {
					$listtitleArray = array('{typename}' => $typeread['typename'], '{sitename}' => $LANPACK['sitename']);
					$LANPACK['sitename'] = $this->fun->formatstring($modelview['listtitlestyle'], $listtitleArray);
				}
			}
		...
		if ($styleid == 1) {
			...
			if (file_exists($tpl_listfile)) {
				...
				for ($index = 1; $index <= $numpage; $index++) {
					...
					while ($rsList = $this->db->fetch_assoc($rs)) {
						...
						$rsList['typelink'] = $this->get_link('type', $typeread_last, $lng_templates);
			...
			if ($typeread['linkid']) {
				...
				$readinfo['pageclass'] = $typeread['pageclass'];
				if ($typeread['nowtid']) {
				...
				if (file_exists($tpl_readfile)) {
					if (!empty($readinfo['headtitle'])) {
						...
						if ($modelview['readtitlestyle']) {
							$readtitleArray = array('{title}' => $readinfo['title'], '{typename}' => $typeread['typename'], '{sitename}' => $LANPACK['sitename']);
	...
	function oncreatedoc() {
		...
		$this->ectemplates->assign('modelarray', $modelarray['list']);
		$this->ectemplates->assign('is_html', $this->CON['is_html']);
		$this->ectemplates->assign('tab', $tab);
		$this->ectemplates->assign('is_alonelng', $this->CON['is_alonelng']);
		$this->ectemplates->assign('home_lng', $lng);
	...
	function oncreatdocsave() {
		$lngpack = $this->fun->accept('lng', 'P');
		$lng = ($lngpack == 'big5') ? $this->CON['is_lancode'] : $lngpack;
		$db_where = ' WHERE isclass=1 AND islink=0 AND ishtml=1 AND isbase=0';
		$mid = $this->fun->accept('mid', 'P');
		if (!empty($mid)) {
			$db_where.=' AND mid=' . $mid;
		...
		$limitnummax = empty($limitnummax) ? 100 : $limitnummax;
		$db_table = db_prefix . 'document';
		$sql = 'SELECT did FROM ' . $db_table . $db_where . ' ORDER BY did DESC LIMIT ' . $limit . ',' . $limitnummax;
		$rs = $this->db->query($sql);
		while ($rsList = $this->db->fetch_assoc($rs)) {
			$this->articlehtml($rsList['did'], $lngpack);
		}
		unset($rsList);
		exit('true');
	}
	...
	function oncreatebatwindow() {
		...
		$read['creattype'] = $this->fun->accept('creattype', 'R');
		$read['isbatdoc'] = $this->fun->accept('isbatdoc', 'R');
	...
	function onstatmidsub() {
		$lng = $this->fun->accept('lng', 'R');
		$lng = ($lng == 'big5') ? $this->CON['is_lancode'] : $lng;
		$db_where = " WHERE isclass=1 AND ishtml=1 AND lng='$lng'";
		$midlist = $this->fun->accept('midlist', 'P');
		if (!empty($midlist)) {
			$db_where.=" AND mid IN($midlist)";
		}
		...
		$sql = 'SELECT sid FROM ' . $db_table . $db_where;
		$rs = $this->db->query($sql);
		while ($rsList = $this->db->fetch_assoc($rs)) {
			$sidlist.=$rsList['sid'] . ',';
		}
	...
	function oncreatsubjectsave() {
		...
		if (!empty($subread['headtitle'])) {
			$LANPACK['sitename'] = $subread['headtitle'];
		} else {
			if ($modelview['listtitlestyle']) {
				$listtitleArray = array('{typename}' => $subread['subjectname'], '{sitename}' => $LANPACK['sitename']);
				$LANPACK['sitename'] = $this->fun->formatstring($modelview['listtitlestyle'], $listtitleArray);
			}
		}
